...
int nAnonymizeAspectCoinAmount = 500;
...
#else
    const char* pszModule = "AspectCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AspectCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AspectCoin
    // Mac: ~/Library/Application Support/AspectCoin
    // Unix: ~/.aspectcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AspectCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "aspect.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "aspectd.pid"));
