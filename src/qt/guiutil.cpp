...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("AspectCoin"))
        return false;
...
{
    // Convert AspectCoin:// to AspectCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("AspectCoin://"))
    {
        uri.replace(0, 12, "AspectCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AspectCoin.lnk";
}
...
{
    return GetAutostartDir() / "AspectCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("AspectCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  AspectCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("AspectCoin-Qt"));
